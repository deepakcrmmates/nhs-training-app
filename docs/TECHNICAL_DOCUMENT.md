...
| 2026-04-22 | Hide Agent Details card on Vendor Availability stage | `nhsApplicationDetailV2.html` "Agent Details / Valuation Figures / Recommendations" card guard changed from `if:true={notFinalChecks}` to `if:true={showAgentCard}`. New getter `showAgentCard` returns `false` when `NHS_Process__c` is either `Vendor Availability` **or** `Final Checks` — showing agent-booking UI before agents have been booked was confusing end users. Section now appears from the `Agents Booked` stage onwards. |
