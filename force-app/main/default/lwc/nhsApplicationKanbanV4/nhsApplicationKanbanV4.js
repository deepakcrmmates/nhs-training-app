...
// Sub-stage pill styling: bg, text, border
const PILL_STYLES = {
...
export default class NhsApplicationKanbanV4 extends NavigationMixin(LightningElement) {
    ...
    // ── Flow pills for header (sub-stages with arrows between) ───────────
    get flowPills() {
        const stages = [
            'To be Contacted', '1st Contact', '2nd Contact', '3rd Contact', 'Sales Cancelled'
        ];
        const items = [];
        stages.forEach((label, i) => {
            ...
            if (i < stages.length - 1) {
                items.push({ key: 'arr-' + i, isArrow: true, label: '' });
            }
        });
        return items;
    }
    ...
    get cancelledCards() {
        const c = this.rawColumns.find(x => x.stageName === 'Sales Cancelled');
        return c ? c.cardCount : 0;
    }
    get activeCards() { return this.totalCards - this.cancelledCards; }
    get contactedCards() {
        // Cards in 1st, 2nd, 3rd Contact combined
