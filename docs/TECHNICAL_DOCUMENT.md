...
| 2026-04-22 | Create Application — soft warnings for recommended-but-not-required fields | `nHSApplicationForm.handleSaveAndSubmit` now calls `collectSoftWarnings()` before submit. Returns labels of any blank/empty recommended fields: **Type of property** (any of Detached/SemiDetached/EndTerrace/MidTerrace/Apartment/Maisonette/Studio/Bungalow/Other), **Number of bedrooms** (`Property.NumberofBedrooms`), **Phone number of Vendor 1** (`Vendor.mobilePhone`), **Email address of Vendor 1** (`Vendor.email`), **Vendor's expectations** (`Application.expectation`). If any are missing, a native `confirm()` dialog lists them and asks the user to continue. Lastname (Vendor 1) remains the only hard-required field. A `warningsAcknowledged` flag stops re-prompting on retry submits in the same session. |
