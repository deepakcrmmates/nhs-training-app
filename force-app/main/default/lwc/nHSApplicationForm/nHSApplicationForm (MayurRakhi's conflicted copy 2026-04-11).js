...
export default class NHSApplicationForm extends LightningElement {
    ...
    get sections() {
        return [
            ...
            { id: 'property-eta', label: 'Property & ETA', icon: 'utility:date_input' },
            { id: 'search-property', label: 'Search Property', icon: 'utility:search' },
    ...
    handleSave() {
        // Simple save without full validation
        let formDataToSave = { ...this.formData };
        
        this.isLoading = true;
        saveData({ formData: formDataToSave, recordType: this.recordType })
            .then(result => {
                const evt = new ShowToastEvent({
                    title: 'Success',
    ...
    handleClickCancel() {
        const evt = new ShowToastEvent({
            title: 'Cancelled',
            message: 'Application form has been cancelled.',
            variant: 'warning',
            mode: 'pester'
        });
        this.dispatchEvent(evt);
