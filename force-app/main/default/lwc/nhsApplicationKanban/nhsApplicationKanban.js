...
const TOP_STAGES = [
    { key: 'application',         label: 'Application',         colour: '#6366f1' },
    { key: 'vendor_availability', label: 'Vendor Availability', colour: '#0ea5e9' },
    { key: 'agents_booked',       label: 'Agents Booked',       colour: '#10b981' },
    { key: 'figures_to_chase',    label: 'Figures to Chase',    colour: '#f59e0b' },
    { key: 'figures_returned',    label: 'Figures Returned',    colour: '#f97316' },
    { key: 'archived',            label: 'Archived',            colour: '#94a3b8' },
];

// Colour map — case-insensitive fallback handled in stageColour()
const SUB_COLOURS = {
...
export default class NhsApplicationKanban extends NavigationMixin(LightningElement) {
    ...
    // KEY FIX: store wire result for refreshApex
    _wiredResult;
    toastTimer;
    ...
    // KEY FIX: cacheable=false — wire re-fetches after refreshApex call
    ...
    wiredData(result) {
        this._wiredResult = result;   // save for refreshApex
    ...
    // KEY FIX: use refreshApex — no page reload needed
    doRefresh() {
        this.isLoading = true;
    ...
    handleDrop(e) {
        ...
        // OPTIMISTIC UPDATE — move card in local state immediately (no flicker)
        ...
        // Persist to Salesforce then sync with refreshApex
        updateOpportunityStage({ opportunityId: cardId, newStageName: targetStage })
            .then(() => {
                this.showToast(`Moved to "${targetStage}"`, 'success');
                this.doRefresh();   // refreshApex keeps local + server in sync
            })
            .catch(err => {
                // Rollback optimistic update on failure
                this.showToast(err.body?.message || 'Move failed — rolling back', 'error');
                this.doRefresh();
            });
    }
