...
| 2026-04-22 | `getEmailTemplates` query across both "NHS Email Templates" folders + `NHS_%` DevName filter | **Root cause of "template #10 doesn't appear in the picker":** there are **two** Salesforce folders both named `NHS Email Templates` — one of Type `EmailTemplate` (Lightning, DeveloperName `NHSEmailTemplates`, holding templates 01–09) and one of Type `Email` (Classic, DeveloperName `NHS_Email_Templates`, where template #10 was deployed via SFDX). The old query used `WHERE Name = 'NHS Email Templates' LIMIT 1` which only picked up the first folder alphabetically, hiding #10. **Fix:** query iterates all matching folders and builds a `Set<Id>`, then queries templates `WHERE FolderId IN :folderIds`. Added `AND DeveloperName LIKE 'NHS_%'` to exclude 4 legacy non-branded templates lurking in the Classic folder. Final picker shows clean 01–10 list. |
